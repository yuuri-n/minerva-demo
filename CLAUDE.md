...
| ⑧ | **ブランド別商談管理デモ** の妥当性確認 → デモ送付済み（2026-05-15時点）。確認ポイント①〜④あり（下記参照） | 新方式採用可否の判断 |
...
- **デモURL（新方式たたき台）**: https://skurashima-cyber.github.io/minerva-demo-v2/demo-v2/index.html
  ...
  - **推奨動線**: ホーム → 取引先「佐藤美容研究所」→ ブランドタブ切替 → 「ブランド別パイプライン」でRTを切替

### ブランド別商談管理 新方式 設計概要（デモ送付済み）

**核心設計**: Opportunityに4つのRecord Typeを定義し、ブランドごとに固有フェーズ列を割り当て

| Record Type | 想定顧客 | フェーズ（順序） |
|---|---|---|
| Minerva | B2C/サロン会員 | 未コンタクト→新規会員登録→アプローチ→相談→提案→受注/購入済→リピート |
| LHALA | B2Bサロン卸 | 資料請求→アプローチ→商談→提案・見積→受注→導入完了 |
| LEMELLA | B2Bサロン卸（説明会あり） | 資料請求→説明会案内→説明会参加→商談→提案・見積→受注→導入完了 |
| FLOWNEE | B2C会員 | 未コンタクト→新規会員登録→案内→相談→購入済→リピート |

**運用イメージ**: 1取引先に対してブランドごとに別商談が並ぶ（同一サロンがMinerva会員＋LHALA卸契約検討中でも取引先1件に集約）

**その他変更点**:
- リードを廃止し、すべて「商談」に一元化
- メールアドレスを識別キーに（名寄せ・重複検知UI付き）
- 商談と購入実績を分離（営業を経ない単発購入もフラグ管理）
- 取引先にランクA〜Dを追加（展示会／キャンペーン向け）
- フェーズ表記を日本語＋確度%を固定値に

**先方確認ポイント（⑧の詳細）**:
1. 各ブランドのフェーズ名・順序・確度% に過不足はないか
2. 代理店様（アイムビー様等）は既存4RTで扱う想定で問題ないか／専用RTを分けた方がよいか
3. B2C個人エンドユーザーも法人サロンと同じ取引先オブジェクトで管理する想定で齟齬はないか
4. 30日経過後の自動フォロー仕様（社内検討中の件）の方向性
