...
| # | 確認内容 | ブロックされる作業 |
|---|---|---|
| ① | アプローチリスト表示条件（最終アプローチ日から何日） | ListView実装・Flowリマインド |
| ② | アプローチリスト除外条件（NGのサブステータス詳細） | 同上 |
| ③ | 商談フェーズ確率（%）の具体値 | PR#8確定値 |
| ④ | 韓国法人売上：スコープ外として進めてよいか | 為替・法人区分の本実装 |
...
| 取引先→キャンペーン登録 | 登録単位（Account/Contact/Lead）＋抽出導線 | Campaign実装 |
| 取引先の漢字名表示・進捗状況 | 項目の正本定義＋ステータス選択肢 | Account FlexiPage |
| 資料請求（Lara）→SF直連携 | Web-to-Lead横展開 vs Element経由 | Laraブランドの流入フロー |
